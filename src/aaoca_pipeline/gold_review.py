...
<div id="message" role="status"></div><main><aside><h2>原始 PDF 切片</h2><p id="source"></p><img id="pdf" alt="原始 PDF 当前页"></aside>
...
CSS = """body{font:15px system-ui,sans-serif;margin:0;color:#182238;background:#f4f6f8}header{display:flex;align-items:center;gap:1rem;background:#152943;color:white;padding:.65rem 1rem;position:sticky;top:0;z-index:3}h1{font-size:1.15rem;margin:0}button{padding:.5rem .8rem;cursor:pointer;border:1px solid #8295a4;border-radius:4px;background:#fff}button:hover{background:#e5f2fc}#message{padding:.4rem 1rem;min-height:1.3rem;color:#8b2100}main{display:grid;grid-template-columns:minmax(380px,50%) 1fr;gap:1rem;padding:1rem}aside,article{background:white;padding:1rem;border-radius:6px;min-width:0}aside{position:sticky;top:4.3rem;height:calc(100vh - 6rem);overflow:auto}#pdf{width:100%;height:auto;border:1px solid #aaa}#source{overflow-wrap:anywhere;font-size:.85rem}textarea{box-sizing:border-box;width:100%;min-height:5rem;font:13px ui-monospace,monospace;padding:.55rem}#gold-text{height:20rem}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:#f1f5f8;padding:.55rem;max-height:18rem;overflow:auto}#line-preview{max-height:14rem}.section{border:1px solid #b6c6d1;border-radius:5px;padding:.7rem;margin:.7rem 0;background:#fbfdfe}.section-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.6rem}.section label{display:flex;flex-direction:column;gap:.2rem}.section input,.section select{padding:.35rem;min-width:0}.section textarea{min-height:3rem}.prediction{padding:.4rem;border-bottom:1px solid #ddd}.actions{display:flex;gap:.7rem;margin:1rem 0}details{margin:.7rem 0}summary{cursor:pointer}@media(max-width:1000px){main{display:block}aside{position:static;height:auto;margin-bottom:1rem}}"""

JS = r"""'use strict';
const token=new URLSearchParams(location.search).get('token');
let data, unitIndex=0, sectionRows=[];
...
async function load(index){try{const info=await api('/api/unit/'+index);data=info;unitIndex=index;$('message').textContent='';$('unit').textContent=`${index+1}/${info.total} · ${info.unit.unit_id} · ${info.unit.stratum} · PDF 第 ${info.unit.page} 页`;$('progress').textContent=`已完成 ${info.completed}/${info.total} 页`;
 ...
 $('pdf').src=`/pdf/${index}?token=${encodeURIComponent(token)}`;
            ...
            try:
                ...
                if prefix == "/pdf/":
                    with fitz.open(source) as pdf:
                        ...
                        pix = pdf[unit["page"] - 1].get_pixmap(matrix=fitz.Matrix(1.35, 1.35), alpha=False)
                ...
                return self.send_json({"unit": unit, "total": len(units), "completed": sum(a["status"] in {"complete", "unreviewable"} for a in annotations.values()),
