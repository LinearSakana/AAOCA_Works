...
<header><h1>Gold Set 人工核验</h1><span id="progress"></span><button id="previous">上一页</button><button id="next">下一页</button><input id="jump" placeholder="页代号 pg_... 或序号"><button id="jump-button">跳到样本</button></header>
...
function jump(){const value=$('jump').value.trim();let index=data.unit_ids.indexOf(value);if(index<0&&/^\d+$/.test(value))index=Number(value)-1;if(index>=0&&index<data.total)load(index);else $('message').textContent='未找到该样本页代号或序号。';}
...
async function load(index){try{const info=await api('/api/unit/'+index);data=info;unitIndex=index;$('message').textContent='';$('jump').value=info.unit.unit_id;$('unit').textContent=`${index+1}/${info.total} · ${info.unit.unit_id} · ${info.unit.stratum} · PDF 第 ${info.unit.page} 页`;$('progress').textContent=`已完成 ${info.completed}/${info.total} 页`;
...
document.addEventListener('DOMContentLoaded',()=>{$('gold-text').oninput=updateLines;$('add-section').onclick=()=>addSection({start_line:1,end_line:1});$('previous').onclick=()=>load(Math.max(0,unitIndex-1));$('next').onclick=()=>load(Math.min(data.total-1,unitIndex+1));$('jump-button').onclick=jump;$('jump').onkeydown=e=>{if(e.key==='Enter')jump();};$('pdf-previous').onclick=()=>showPdfPage(shownPdfPage-1);$('pdf-next').onclick=()=>showPdfPage(shownPdfPage+1);$('pdf-current').onclick=()=>showPdfPage(data.unit.page);$('save-draft').onclick=()=>save('draft');$('save-complete').onclick=()=>save('complete');$('unreviewable').onclick=()=>save('unreviewable');const requested=new URLSearchParams(location.search).get('unit');load(0).then(()=>{if(requested){$('jump').value=requested;jump();}});});
"""
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def route(self, post: bool = False):
            ...
            try:
                ...
                return self.send_json({"unit": unit, "unit_ids": [u["unit_id"] for u in units],
